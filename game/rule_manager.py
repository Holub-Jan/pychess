...
class RuleManager:
    ...
    def _get_knight_top(self, x, y):
        lst = list()
        if x > 0:
            if y < 6 and self.board[x - 1][y + 2].color != self.color:
                lst.append((x - 1, y + 2))
            if y > 1 and self.board[x - 1][y - 2].color != self.color:
                lst.append((x - 1, y - 2))
    ...
    def _get_knight_right(self, x, y):
        lst = list()
        if x < 7:
            if x < 6 and self.board[x + 2][y + 1].color != self.color:
                lst.append((x + 2, y + 1))
            if x > 1 and self.board[x - 2][y + 1].color != self.color:
                lst.append((x - 2, y + 1))
